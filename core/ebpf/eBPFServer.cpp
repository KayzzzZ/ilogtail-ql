...
void eBPFServer::GenerateMetric(logtail::QueueKey key, uint32_t idx) {
    ...
    while (mGenerateFlag) {
        ...
        for (size_t i = 0; i < app_ids.size(); i ++) {
            ...
            mTestEventGroup.SetTag(std::string("service"), std::string(app_names[i]));
            ...
            mTestEventGroup.SetTag(std::string("host"), std::string("test-http-server-1"));
            ...
            for (size_t j = 0 ; j < app_metric_names.size(); j ++) {
                for (size_t z = 0; z < 10; z ++ ) {
                    ...
                    metricsEvent->SetTimestamp(seconds, nano);
                }
            }
        ...
        for (size_t i = 0; i < app_ids.size(); i ++)  {
            ...
            mTestEventGroup.SetTag(std::string("host"), std::string("test-tcp-1"));
            mTestEventGroup.SetTag(std::string("data_type"), std::string("metric"));
            for (size_t j = 0 ; j < tcp_metrics_names.size(); j ++) {
                for (size_t z = 0; z < 20; z ++ ) {
                    ...
                    metricsEvent->SetTag(std::string("destWorkloadName"), std::string("arms-oneagent-test-ql-dst"));
                    metricsEvent->SetTag(std::string("destWorkloadKind"), std::string("qianlu-dst"));
        ...
        for (size_t i = 0; i < items.size(); i ++) {
            ...
            if (status) {
                ...
                LOG_INFO(sLogger, ("[Metrics] push queue success! idx:", idx));
            }
        }
...
void eBPFServer::GenerateSpan(logtail::QueueKey key, uint32_t idx) {
    ...
    while (mGenerateFlag) {
        ...
        for (size_t i = 0; i < app_ids.size(); i ++) {
            ...
            mTestEventGroup.SetTag(std::string("host.name"), std::string("test-host-name"));
...
void eBPFServer::GenerateAgentInfo(logtail::QueueKey key, uint32_t idx) {
    ...
    std::vector<std::string> app_ids = {
        "73994b260f0a33d373f79a1997af2ae5", 
        "500e20a3b7a52f5069cd5a00b20698fe",
        "e0390976c7923158f398c01362cd64f1",
        "ggxw4lnjuz@237e23768f7ca28",
        "ggxw4lnjuz@c65bacfa865451a",
    };
    std::vector<std::string> service_name = {
        "ebpf-test", 
        "mall-gateway-cmonitor", 
        "kube-state-metrics", 
        "test-pre",
        "lixin-demo",
    };
    ...
    while(mGenerateFlag) {
        ...
        for (size_t i = 0; i < app_ids.size(); i ++) {
            ...
            logEvent->SetContent(pidKey, app_ids[i]);
            logEvent->SetContent(appNameKey, service_name[i]);
            logEvent->SetContent(ipKey, "192.168.ql-" + std::to_string(i));
            logEvent->SetContent(hostNameKey, "hostname-ql-" + std::to_string(i));
            logEvent->SetContent(agentVersionKey, "ebpf-0.0.1");
