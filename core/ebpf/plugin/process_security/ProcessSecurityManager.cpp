...
ProcessSecurityManager::ProcessSecurityManager(std::shared_ptr<BaseManager>& baseMgr,
    ...
    : AbstractManager(baseMgr, sourceManager, queue, scheduler),
      mAggregateTree(
          4096,
          [this](std::unique_ptr<ProcessEventGroup>& base, const std::shared_ptr<CommonEvent>& other) {
              base->mInnerEvents.emplace_back(std::move(other));
          },
          [this](const std::shared_ptr<CommonEvent>& in) {
...
bool ProcessSecurityManager::ConsumeAggregateTree(const std::chrono::steady_clock::time_point& execTime) {
    ...
    WriteLock lk(mLock);
    SIZETAggTree<ProcessEventGroup, std::shared_ptr<CommonEvent>> aggTree(this->mAggregateTree.GetRootNodeAndClear());
    lk.unlock();
