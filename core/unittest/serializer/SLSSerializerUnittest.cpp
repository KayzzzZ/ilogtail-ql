...
private:
    ...
    BatchedEvents CreateBatchedSpanEvents(bool enableNanosecond, uint32_t nanoTimestamp, bool emptyValue);
...
void SLSSerializerUnittest::TestSerializeEventGroup() {
    ...
    }
    {
        // span event
        string res, errorMsg;
        APSARA_TEST_TRUE(serializer.DoSerialize(CreateBatchedSpanEvents(false, 0, false), res, errorMsg));
        sls_logs::LogGroup logGroup;
        APSARA_TEST_TRUE(logGroup.ParseFromString(res));
        APSARA_TEST_EQUAL(1, logGroup.logs_size());
    }
...
BatchedEvents SLSSerializerUnittest::CreateBatchedSpanEvents(bool enableNanosecond, uint32_t nanoTimestamp, bool emptyValue) {
    PipelineEventGroup group(make_shared<SourceBuffer>());
    group.SetTag(LOG_RESERVED_KEY_TOPIC, "topic");
    group.SetTag(LOG_RESERVED_KEY_SOURCE, "source");
    group.SetTag(LOG_RESERVED_KEY_MACHINE_UUID, "machine_uuid");
